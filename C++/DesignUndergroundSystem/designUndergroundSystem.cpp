...
public:
    unordered_map<int, pair<string, int>> checkInTable; //id-> {stationName, time}
    unordered_map<string, pair<double, int>> routeTable; //route->{time, count}
    UndergroundSystem() {
    }
    
    void checkIn(int id, string stationName, int t) {
        checkInTable[id] = {stationName, t};
    }
    
    void checkOut(int id, string stationName, int t) {
        auto checkInDetails = checkInTable[id];
        //remove this from map i.e., check out
        checkInTable.erase(id);
        string routeName = checkInDetails.first + "->" + stationName;
        routeTable[routeName].first += t - checkInDetails.second;
        routeTable[routeName].second += 1;
        
    }
    
    double getAverageTime(string startStation, string endStation) {
        string routeName = startStation + "->" + endStation;
        auto& route = routeTable[routeName];
