...
public:
    vector<int> pivotArray(vector<int>& nums, int pivot) {
        //Idea is to implement a modified 2-ptr method
        //3 ptrs are needed to account for multiple val == pivot
        int low = 0, pvt = 0, high = nums.size();
        for(int n: nums){
            if(n < pivot)
                ++low;
            else if(n == pivot)
        ...
        //higher values start after low+pivot values
        //equal values start after all lower values
        high = pvt + low;
        pvt = low;
        low = 0;
        for(int i: nums){
